...
- **도입 문턱을 숫자로 적었다.** 「서버 한 대와 도메인이면 시작합니다」 — compose 한 벌에 리버스 프록시(Caddy)를
  얹으면 HTTPS 인증서는 자동 발급이고, 별도 DB 서버·메시지 큐가 없다(이 사이트도 작은 VM 한 대에서 돈다).
- **도구 개발은 바이브 코딩으로.** 저장소에 기존 어댑터와 에이전트용 규약(`CLAUDE.md`)이 함께 있어, 코딩
  에이전트에게 「이 API 를 도구로 붙여 줘」라고 시키면 초안이 나온다 — 인증·권한·궤적은 허브가 이미 맡고 있고
  새로 쓰는 것은 상류 호출과 응답 정리뿐이라는 점을 명시했다.

## [0.5.4] — 2026-09-20

### 변경

- **「사내에서 쓰기」 → 「원하시는 도구를 손쉽게 개발·운영하시려면」.** 「계속 쓰려면 설치하라」 는 소극적이었다 —
  잇다 허브가 **필요한 도구를 직접 만들어 붙이는 플랫폼**이라는 것이 핵심이라, 절 제목과 본문을 그쪽으로 다시 썼다
  (도구 하나 = 파일 하나 · 표준 MCP 연결 · 도구함 단위 권한과 궤적 · 데이터는 사내에 · compose 한 벌).
- **고지 띠의 링크가 GitHub 로 튕기지 않는다.** 처음 온 사람을 저장소로 보내면 맥락 없이 코드를 마주한다 —
  이제 같은 화면의 안내 절(`#selfhost`)로 스크롤하고, 그 절이 `:target` 으로 한 번 번쩍인다(스크립트 없이 CSS 만,
  `prefers-reduced-motion` 에서는 움직임 없음). 저장소·문의 버튼은 그 절 안에 있다.

## [0.5.3] — 2026-09-20

### 추가

- **도입 문의 경로.** 「사내에서 쓰기」 절에 `dev@itda.work` 로 가는 「도입 문의」 버튼(제목이 채워진 mailto)과
  본문 링크를 뒀다. 설치 문서만 두면 막히는 자리에서 물어볼 곳이 없다.

## [0.5.2] — 2026-09-20

### 추가

- **사내 도입 안내.** 고지 띠에 「업무에 계속 쓰시려면 오픈소스 허브를 사내에 직접 설치하세요」 한 줄과 저장소 링크를
  더하고, 도구함에 「사내에서 쓰기」 절을 뒀다(MIT · compose 한 벌 · 로그인 수단 선택 · 도구를 어댑터 한 파일로 확장).
  고지만 하고 대안을 주지 않으면 막다른 길이다 — 시험 운영이라는 사실과 그래서 어떻게 하면 되는지를 같은 자리에서 본다.

## [0.5.1] — 2026-09-20

### 추가

- **시험 서비스 고지 띠**(모든 화면 상단, 헤더 위). 「이곳의 도구는 실험적으로 제공되며 예고 없이 변경되거나
  중단될 수 있다」 는 것을 도구를 담고 연결하는 사람이 매번 보게 한다. **닫는 버튼을 두지 않는다** — 한 번 닫으면
  그 브라우저에서 영영 안 보이는데, 이건 배너가 아니라 서비스의 전제다.
