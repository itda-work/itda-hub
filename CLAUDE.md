...
- **연결 토큰은 DOT `AccessToken` 을 그대로 쓴다**(`apps/toolbox/tokens.py`). 별도 토큰 모델·별도 검증 경로를 만들지 않는다 — MCP 서버가 OAuth 토큰과 연결 토큰을 구분하지 않는 것이 계약이다. 원문은 체크섬만 저장(RFC 9700). **사용자 화면에는 OAuth 커넥터만 노출한다** — 연결 토큰 발급은 `issue_token` 커맨드(운영·스모크)뿐.
...
- 비밀은 환경변수로만. 저장소에는 `.env.example` 과 가상 시드만. 도구 설정 값은 암호화 저장하고 화면·로그·궤적에 평문을 남기지 않는다. 토큰 원문을 찍는 곳은 `issue_token` 의 stdout 한 곳뿐이다(화면 발급 경로는 0.4.0 에서 없앴다).
